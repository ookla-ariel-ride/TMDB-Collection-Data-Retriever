...
TMDB_COLLECTION = '%s/collection/%%s?api_key=%s&language=%%s' % (
    TMDB_URL, TMDB_APIKEY)
TMDB_COLLECTION_IMG = '%s/collection/%%s/images?api_key=%s' % (
    TMDB_URL, TMDB_APIKEY)
...
def main():
    ...
    input_sections = ''.join(
        i for i in input_sections if i.isdigit() or i.isspace()).split()
    ...
    for section_id in input_sections:
        ...
        print(''.ljust(80, '='))
        print('Library: %s (%s collections)' %
              (section_dict[section_id], len(plex_all_col_xml)))
        print(''.ljust(80, '='))
        ...
        for plex_col_xml in plex_all_col_xml:
            ...
            print(
                '\r\n> %s [%s/%s]' %
                (plex_col_dict['title'], i, len(plex_all_col_xml)))
            ...
            plex_col_mov_xml = GetPlexData(
                PLEX_COLLECTIONS_ITEMS % plex_col_id)
            ...
            print(
                ...
                (len(
                    tmdb_col_img_dict['posters']), len(
                    tmdb_col_img_dict['backdrops'])))

            poster_url_list = GetImages(
                tmdb_col_img_dict,
                tmdb_conf_dict,
                'posters',
                lang,
                POSTER_ITEM_LIMIT)
            background_url_list = GetImages(
                tmdb_col_img_dict,
                tmdb_conf_dict,
                'backdrops',
                lang,
                BACKGROUND_ITEM_LIMIT)
...
def UploadImagesToPlex(url_list, plex_col_id, image_type, image_type_name):
    if url_list:
        ...
        for background_url in url_list:
            ...
            if plex_main_image == '':
                plex_main_image = GetPlexPosterUrl(
                    PLEX_IMAGES %
                    (plex_col_id, image_type + 's', background_url))
...
def GetTMDBCollectionID(plex, mov_in_col_xml):
    for mov_in_col in mov_in_col_xml:
        ...
        if movie.guid.startswith(
                'com.plexapp.agents.imdb://'):  # Plex Movie agent
            match = re.search(r"tt[0-9]\w+", movie.guid)
        # TheMovieDB agent
        elif movie.guid.startswith('com.plexapp.agents.themoviedb://'):
        ...
        if movie_dict and 'belongs_to_collection' in movie_dict and movie_dict[
            ...
            print(
                ...
                (col_id, movie.title, movie_id, lang))
            return col_id, lang
...
def GetTMDBData(url):
    ...
    for i in range(tries):
        try:
            ...
            if enable_debug:
                print('Requests in time limit remaining: %s' %
...
def GetImages(img_dict, conf_dict, type, lang, artwork_item_limit):
    ...
    if img_dict[type]:
        ...
        while i < len(img_dict[type]):
            ...
            # boost the score for localized posters (according to the
            # preference)
        ...
        for i, poster in enumerate(
                sorted(img_dict[type], key=lambda k: k['vote_average'], reverse=True)):
            if i >= artwork_item_limit:
                break
            else:
                result.append(
                    conf_dict['images']['base_url'] +
                    'original' +
                    poster['file_path'])
